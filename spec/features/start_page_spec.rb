...
RSpec.feature "StartPage", type: :feature do
  ...
  describe "toolbar" do
  	it "suppose sign in or register to guest user", :js => true do
  		visit "/"
  		expect(page).to have_text("Bitte melden Sie sich an oder registrieren !!!")
  	end
  	it "shows brand", :js => true do
  		visit "/"
  		expect(page.find('.md-toolbar-tools')).to have_text("DeBug")
  	end
  	it "shows login/register menu for guest user", :js => true do
  		visit "/"
  		expect(page.find('.md-toolbar-tools')).to have_button("Log In")
  		expect(page.find('.md-toolbar-tools')).to have_button("Register")
  	end
  	it "doesn't show logout button for guest user", :js => true do
  		visit "/"
